    ...
    if info.get("logged_in"):
        return {
            ...
            "instruction": [
                ...
                "Call tool get_permissions and get_limits to show user permissions and limits (Re-use the result if user already called these tools before)",
            ],
            "trace_id": session_handle,
        }
...
async def validate_upload_product_list_excel_data(
    ...
    if upload_status == "processing" and upload_info.get("mapped_data"):
        ...
        wish_list_id = upload_info.get("wish_list_id")
        if wish_list_id:
            return {
                "status": "success",
                "code": "UPLOAD_EXTRACTING",
                ...
                "instruction": [
                    "Already extracting, call check_upload_product_list_excel_data_status tool with wish_list_id to continue",
                ],
                "trace_id": session_handle,
            }
...
async def check_upload_product_list_excel_data_status(
    ...
    When not to call:
        - When can not define the wish_list_id from chat context (Ask user to provide).
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "instruction": [
                    ...
                    "If progress is finished and there are unmatched products, suggest user to list them by calling get_sds_request tool with wish_list_id from data."
                ],
                "trace_id": session_handle,
            }
        else:
...
async def get_sds_request(
    ...
    recommend_instruction = [
        "Display results in table",
        "If no results are found, suggest user to upload_product_list_excel_file tool for uploading another product list",
