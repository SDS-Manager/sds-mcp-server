...
**Mode 1 — Post-coding task (skip interview):** When documenting work already done by Claude (code written, PR exists). Skip RIRE scoring and interview. Create the task with a clear description of what was done, link to the PR, and files changed. Prefix title with `CLAUDE CODE:` and set status to `ADD NEW TASKS HERE`.

**Mode 2 — Request-phase task (full process):** When someone requests NEW work that hasn't started. Follow the full process below. Do NOT prefix the title with `CLAUDE CODE:`.
...
1. **Interview you** — ask clarifying questions until the task is fully specified
2. **Flag schema or package changes** — if the task needs new DB columns or packages, Claude will highlight this with `⚠️ DATABASE CHANGES REQUIRED` or `⚠️ NEW PACKAGES REQUIRED` at the top of the description
3. **Show you a preview** of the full task (title + description) and ask for confirmation before creating it
4. **Create the task** in ClickUp with all required fields
...
| Other | Anything not listed above (use for MCP server tasks) |
...
Claude will **propose** a priority based on your RIRE score and show: "Based on your RIRE score of X, I'd suggest **PY**. Does that look right, or would you like to override?"

You confirm or override. **Exception:** if you say "production bug" or "deal blocked", Claude uses P0/P1 regardless of RIRE score.
...
## Affected Area
[Frontend / Backend / MCP Server / Full stack / etc.]
...
## Technical Context
[Relevant endpoints, tables, components, links — or "None"]
```
